...
| `extra` | `26102 + 500 = 26602` | ADR-013 fields: department, labour, demandedDays, returnedRs |
...
## 2. Works generation (144 works, ids W-1001..W-1144)
...
1. `main` rng draws the status shuffle: Fisher-Yates `shuffled()` over the 143-work rest list
   (66 in-execution + 26 stalled + 26 completed + 25 sanctioned) — 142 internal draws, pre-loop.
2. Loop `n = 1001..1144` (`index = n − 1001`); `place = DISTRICTS[index % 12]` (12 works per district).
3. **W-1014 is flagship: every field pinned, zero rng** —
   `type=community-hall, status=stalled, title="Community Hall — Ward Block 7",
   agency=Contractor-07, sanctionedRs=19000000 (₹1.90 Cr), expenditureRs=13300000, progressPct=62,
   ...
   department=PWD, labourDeployed=42, demandedDays=330, returnedRs=0`.
...
4. Non-flagship works, in TS source order per work:
   ...
   - `sanctionedRs = round10k(800000 + main()*27200000)` — ₹8L..₹2.8Cr, nearest ₹10k
     (the rupee mirror of the old `oneDecimal` 0.1L precision)
   - `progressPct = progressFor(main, status)` — completed→100; sanctioned→int(0,5); in-execution→int(15,85); stalled→int(10,70)
   - `expenditureRs = round10k(sanctionedRs * spendRatioFor(main, status))` — completed→0.95+main()*0.05; sanctioned→main()*0.1; else→0.2+main()*0.65
   ...
   - `extra` stream, in order: `department = DEPARTMENTS[int(extra, 0, 3)]`,
     ...
     `returnedRs = round10k(min(extra() * min(1200000, sanctionedRs * 0.15), headroom))`
     where `headroom = max(sanctionedRs − expenditureRs, 0)`; else `returnedRs = 0`
...
3. `FLAG_PLAN` in order: cost/high, cost/high, duplicate/high, delay/high, expenditure/high,
   ...
   utilisation/low, then cost/high, duplicate/high, delay/high, expenditure/medium,
   cost/medium, utilisation/medium, delay/medium, cost/low, expenditure/low, delay/low,
   utilisation/low, duplicate/medium (8 high total).
4. Flag `i` (0-based) creates stream `mulberry32(26102 + i*101)`; inside `buildAnomaly`:
   ...
   (cost ratio: flagship pinned 19000000/7800000, high `2.0 + rng()*0.6`, else `1.4 + rng()*0.5`;
   expenditure ratio: high `1.6 + rng()*0.6`, else `1.2 + rng()*0.2`;
   utilisation band `0.55 + rng()*0.2`); money medians via `round10k` (₹10k precision).
   Port the branch logic verbatim from
   `buildAnomaly` in the TS file; text templates come from `anomaly-engine.md`.
5. Ids `A-1..A-24` in flag order (A-1 = the W-1014 cost flag).
...
- **Evidence** (`evidence` stream 26123): 6 fixed files in order —
  ...
  `workId = anomalies[index % 24].workId`, `sizeKb = int(rng, 180, 4200)`,
...
- **Activities** (no rng): per anomaly index i, in order —
  ...
  3. note row (only i < 16): at `DEMO_TODAY T08:15:00.000Z`, actor `District Officer (demo)`,
  ...
  Counter `T-1..` increments in generation order → 65 rows total (24×2 + 16 + 1).
...
Exactly: `totalWorks=28410, underExecution=9120, delayed=1140, highRisk=214,
overrunExposureRs=1284000000` (₹128.40 Cr). Served by `GET /overview/kpis` (api-reference.md) until real ingest.
...
3. Deep-equal assert: all 144 works (21 fields each), 24 anomalies incl. `signals` arrays,
   6 evidence rows, 65 activity rows, 6-state geo rollup. Money compared as integer rupees;
