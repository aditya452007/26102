...
class Work(db.Entity):
    ...
    agency = Required(str)
    status = Required(str)
    sanctioned_lakh = Required(Decimal)
    expenditure_lakh = Required(Decimal)
    ...
    returned_lakh = Required(Decimal, default=Decimal("0"))
    created_at = Required(datetime, default=_now)
...
class Anomaly(db.Entity):
    ...
    peer_n = Required(int)
    peer_median_lakh = Optional(Decimal, nullable=True)
    actual_lakh = Optional(Decimal, nullable=True)
    unit = Required(str, default="₹L")
